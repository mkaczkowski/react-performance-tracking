...
| `lighthouse` + `chrome-launcher` | Using Lighthouse audits (`npm install -D lighthouse chrome-launcher`) |

:::info
All peer dependencies are optional. Install only what you use. Lighthouse audits require both `lighthouse` and `chrome-launcher`.
:::
