...
thresholds: {
  base: {
    ...
    webVitals?: { lcp?: number, inp?: number, cls?: number, ttfb?: number, fcp?: number },
    longTasks?: { tbt?: number, maxDuration?: number, maxCount?: number },
...
interface WebVitalsThreshold {
  lcp?: number;   // ms - Largest Contentful Paint (Google recommends ≤2500ms)
  inp?: number;   // ms - Interaction to Next Paint (Google recommends ≤200ms)
  cls?: number;   // score - Cumulative Layout Shift (Google recommends ≤0.1)
  ttfb?: number;  // ms - Time to First Byte (Google recommends ≤800ms)
  fcp?: number;   // ms - First Contentful Paint (Google recommends ≤1800ms)
}
```
...
```ts
interface LongTasksThreshold {
  tbt?: number;         // ms - Total Blocking Time (sum of blocking time above 50ms)
  maxDuration?: number; // ms - Maximum duration of any single long task
  maxCount?: number;    // Maximum number of long tasks allowed
}
```
...
| Long Tasks | `longTasks` threshold configured | Chromium |
...
| Long Tasks | Additive | 200ms TBT + 20% = 240ms max |
